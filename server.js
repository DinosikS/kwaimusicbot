...
require('http').createServer((req, res) => res.end('Kwai Music online!')).listen(3000)


client.on
client.on("ready", () => {
    client.user.setActivity("k.help| k.invite", { type: "WATCHING"})
})
...
console.log("Subscribe on Dinosik")
...
client.on("message", async message => {
  if (message.channel.name == "abotchat") {
    ...
    message.channel.startTyping();
    if (!message.content) return message.channel.send("Please say something.");
    scb.chat({message: message.content, name: client.user.username, owner:"Dinosik", user: message.author.id, language:"auto"}).then(reply => {
